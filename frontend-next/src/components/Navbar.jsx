                ...
                <div className="flex justify-between items-center h-24">
                    ...
                    <div className="hidden lg:flex items-center gap-6">
                        {mounted && isAuthenticated ? (
                            ...
                            <div className="flex items-center gap-4 border-l border-gray-200 pl-6">
                                <Link href="/login" className="text-sm font-bold text-[var(--color-text)] hover:text-[var(--color-primary)] transition-colors">
                                    Log In
                                </Link>
                                <Link href="/register">
                        ...
                        >
                            ...
                            {mounted && items.length > 0 && (
                                <span className="absolute -top-1 -right-2 flex h-5 w-5 items-center justify-center rounded-full bg-[var(--color-primary)] text-xs font-bold text-white shadow-sm ring-2 ring-white">
                                    {items.length}
                                </span>
                            )}
                        </Link>
                    ...
                    <div className="flex lg:hidden items-center gap-4">
                        <Link href="/cart" className="relative text-[var(--color-text)]">
                            <FiShoppingBag size={28} />
                            {mounted && items.length > 0 && (
                                <span className="absolute -top-1 -right-2 flex h-5 w-5 items-center justify-center rounded-full bg-[var(--color-primary)] text-xs font-bold text-white shadow-sm ring-2 ring-white">
                                    {items.length}
            ...
            >
                <div className="flex flex-col h-full pt-24 pb-10 px-8 overflow-y-auto">
                    ...
                    <div className="mt-auto border-t border-gray-100 pt-8 flex flex-col gap-4">
                        {mounted && isAuthenticated ? (
                            <>
                                <Link href="/profile" onClick={() => { scrollToTop(); setIsOpen(false); }} className="flex items-center gap-3 text-lg font-medium">
                                    <FiUser /> My Profile
                                </Link>
                                <Link href="/my-orders" onClick={() => { scrollToTop(); setIsOpen(false); }} className="flex items-center gap-3 text-lg font-medium">
                                    <FiPackage /> My Orders
                                </Link>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <Link href="/register" onClick={() => { scrollToTop(); setIsOpen(false); }}>
